CREATE OR REPLACE PACKAGE BODY CB_RennequinepolisCBLight AS
	...
	PROCEDURE VerificationQuotationOpinion(P_QUOTA QuotationsOpinions.Quotation%TYPE, P_OPIN QuotationsOpinions.Opinion%TYPE) AS
		...
		V_Logs.LogWhere := 'CB_RennequinepolisCBLight.VerificationQuotationOpinion';
		AddLogs(V_Logs);
	EXCEPTION
		WHEN E_QuotaOpinNull THEN
			...
			V_Error.LogWhere := 'CB_RennequinepolisCBLight.VerificationQuotationOpinion';
			AddLogs(V_Error);
		WHEN OTHERS THEN
			V_Error.LogWhen := CURRENT_DATE;
			V_Error.ErrorCode := SQLCODE;
