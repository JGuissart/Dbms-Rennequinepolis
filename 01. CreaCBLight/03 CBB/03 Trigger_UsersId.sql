...
FOR EACH ROW
	DECLARE
		...
		V_Logs CBB_RennequinepolisCBLight.R_Logs;
		V_Error CBB_RennequinepolisCBLight.R_Logs;
	BEGIN
		IF(:NEW.DateOfBirth > TO_DATE(CURRENT_DATE, 'DD/MM/YY')) THEN
			RAISE E_InvalidDate;
		END IF;
		:NEW.idUser := CBB_RennequinepolisCBLight.GenerateIdUser(:NEW.LastName, :NEW.FirstName, :NEW.DateOfBirth);
	...
	EXCEPTION
		WHEN E_InvalidDate THEN
			...
			CBB_RennequinepolisCBLight.AddLogs(V_Error);
		WHEN OTHERS THEN
			V_Error.LogWhen := CURRENT_DATE;
