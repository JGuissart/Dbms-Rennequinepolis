...
WHEN (NEW.Token IS NULL) -- Token null => la cote et/ou l'avis du film n'a pas encore été répliquée sur CBB
	...
	BEGIN
		...
		IF(V_CheckUser = TRUE) THEN -- Si la fonction a retourné TRUE, on réplique via le trigger
			IF INSERTING THEN
				INSERT INTO QuotationsOpinions@CB VALUES(:NEW.Login, :NEW.idMovie, :NEW.Quotation, :NEW.Opinion, :NEW.DateOfPost, :NEW.Token);
				AddLogs('INSERT le tuple identifié par le couple Login/IdMovie ' || :OLD.Login || '/' || :OLD.idMovie || ' dans la table QuotationsOpinions de CB', 'Trigger ReplicationCotesAvis', NULL);
			END IF;
			IF UPDATING THEN
				...
				AddLogs('UPDATE le tuple identifié par le couple Login/IdMovie ' || :OLD.Login || '/' || :OLD.idMovie || ' dans la table QuotationsOpinions de CB', 'Trigger ReplicationCotesAvis', NULL);
			END IF;
			IF DELETING THEN
				DELETE FROM QuotationsOpinions@CB WHERE Login = :OLD.Login AND idMovie = :OLD.idMovie;
				AddLogs('DELETE le tuple identifié par le couple Login/IdMovie ' || :OLD.Login || '/' || :OLD.idMovie || ' dans la table QuotationsOpinions de CB', 'Trigger ReplicationCotesAvis', NULL);
			END IF; 
		ELSE -- Sinon, la cote et/ou l'avis sera répliqué via la restauration
	...
	EXCEPTION
		WHEN OTHERS THEN
			AddLogs('OTHERS : ' || SQLERRM, 'Trigger ReplicationCotesAvis', SQLCODE);
END;
/