create or replace PACKAGE BODY BackupCB AS
	...
	FUNCTION CheckUserToReplicate(P_LOGIN IN Users.Login%TYPE) RETURN BOOLEAN AS
		...
		BEGIN
			AddLogs('Début fonction CheckUserToReplicate', 'BackupCB.CheckUserToReplicate', NULL);
			SELECT * INTO V_User
			FROM Users@CBB
			WHERE Login = P_LOGIN;
			AddLogs('Fin fonction CheckUserToReplicate', 'BackupCB.CheckUserToReplicate', NULL);
			RETURN TRUE;
		EXCEPTION
	...
	FUNCTION CheckMovieToReplicate(P_IDMOVIE IN Movies.IdMovie%TYPE) RETURN BOOLEAN AS
		...
		BEGIN
			AddLogs('Début fonction CheckMovieToReplicate', 'BackupCB.CheckMovieToReplicate', NULL);
			SELECT * INTO V_Movie
			FROM Movies@CBB
			WHERE IdMovie = P_IDMOVIE;
			AddLogs('Fin fonction CheckMovieToReplicate', 'BackupCB.CheckMovieToReplicate', NULL);
			RETURN TRUE;
		EXCEPTION
			WHEN NO_DATA_FOUND THEN
				RETURN FALSE;
	...
	PROCEDURE Job_ReplicationAsync AS
		...
		EXCEPTION
			WHEN OTHERS THEN
				ROLLBACK;
				AddLogs('OTHERS : ' || SQLERRM, 'BackupCB.Job_ReplicationAsync', SQLCODE);
