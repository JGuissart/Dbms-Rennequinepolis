...
BEGIN
	...
	IF(V_FuncReturn = TRUE) THEN -- Si la fonction a retourné TRUE, on ne réplique pas via le trigger ! Ce sera répliqué via le JOB
		IF INSERTING THEN
	...
	ELSE
		IF INSERTING THEN
			INSERT INTO QuotationsOpinions@CBB VALUES(:NEW.idUser, :NEW.idFilm, :NEW.Quotation, :NEW.Opinion, :NEW.DateOfPost, :NEW.Token);
			V_Logs.LogWhen := CURRENT_DATE;
			V_Logs.ErrorCode := NULL;
		...
		END IF;
		IF DELETING THEN
			DELETE FROM QuotationsOpinions@CBB WHERE idUser = :OLD.idUser AND idFilm = :OLD.idFilm;
			V_Logs.LogWhen := CURRENT_DATE;
			V_Logs.ErrorCode := NULL;
